%% Logistic other flat prior, to show prior predictive distribution
...
modelName = 'LogisticOtherFlatPrior';
...
rng(13); % set random number seed so get same posterior samples
...
alphaRange = [-3e3 3e3]; % axis limits for parameter space
betaRange = [-3e3 3e3];
...
for i = 1:nDraw
   ...
   H = plot(x, 1./(1 + exp(-beta*(x-standard-alpha))), 'k-');
